...
class RetailPipelineStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs):
        ...
        # =====================================================
        # SNOWFLAKE S3 READ USER (access keys → Snowflake stage)
        # =====================================================
        snowflake_user = iam.User(
            self,
            "SnowflakeS3ReaderUser",
            user_name="snowflake-s3-reader",
        )

        snowflake_user.add_to_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=["s3:GetObject", "s3:GetObjectVersion"],
                resources=[f"{processed_bucket.bucket_arn}/*"],
            )
        )
        snowflake_user.add_to_policy(
            iam.PolicyStatement(
        ...
        CfnOutput(
            self,
            "SnowflakeUserArn",
            value=snowflake_user.user_arn,
            description="After deploy: IAM → Users → snowflake-s3-reader → Create access key, then use keys in Snowflake stage CREDENTIALS",
            export_name="RetailPipelineStack-SnowflakeUserArn",
        )
